resource "aws_instance" "public_ec2" {
  ...
  subnet_id              = module.olympo_vpc.public_subnet[0] # aws_subnet.public_subnet.id
...
resource "aws_instance" "monitoring_ec2" {
  ...
  ami                    = var.ec2_spec.ami
  instance_type          = var.ec2_spec.instance_type
  subnet_id              = module.olympo_vpc.public_subnet[0]
  key_name               = data.aws_key_pair.lange2_key.key_name
  vpc_security_group_ids = [module.olympo_vpc.sg_public_instance[0]]
  user_data              = file("scripts/userdata.sh")
